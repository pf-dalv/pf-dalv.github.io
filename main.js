...
(function checkCSSLoadingIssues() {
    console.log("Checking possible reasons why CSS might not be loading...");

    // Check if stylesheets exist in the document
    if (document.styleSheets.length === 0) {
        console.warn("No stylesheets detected. Ensure your CSS file is linked in the HTML.");
    }
    ...
    Array.from(document.styleSheets).forEach((sheet, index) => {
        try {
            ...
            if (!sheet.href) {
                console.warn(`Stylesheet at index ${index} is inline.`);
            } else if (sheet.cssRules.length === 0) {
                console.warn(`Stylesheet ${sheet.href} has no CSS rules. Check if the file exists and is not empty.`);
            }
        } catch (error) {
            console.error(`Error accessing stylesheet ${sheet.href}:`, error);
            console.warn("Possible causes: CORS restrictions, incorrect file path, or file not found.");
        }
    });

    // Check if the <link> elements exist and have valid hrefs
    document.querySelectorAll("link[rel='stylesheet']").forEach((link, index) => {
        if (!link.href) {
            console.warn(`Stylesheet link at index ${index} has an empty href.`);
        } else if (!link.sheet) {
            console.error(`Stylesheet ${link.href} failed to load. Check the file path, network errors, or CORS issues.`);
        }
    });

    console.log("CSS loading check complete.");
})();
